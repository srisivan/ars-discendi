...
- Process: Take a problem, with its own set of requirements, and:
	- iteratively break it down into smaller unit components which can be easily managed and designed;
	- make sure all these components work in harmony
	- assemble all the unit components and watch them work and achieve the overall result, within the specified requirements. 
- Pre-requisites:
	- Survey any and all existing systems (if available) which are already serving the problem. Find and define any deficiencies, and plans to mitigate.
	- Create a plan for the new system, confirm that it is compatible with the established requirements. Test it for the same. 
	- Publish.
	- It is an iterative process, which focuses more on maintaining and damage control as much as on designing systems.
- Phase in software development process which is aimed at designing high-level design of the software system. 
